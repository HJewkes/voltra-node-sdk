/**
 * bulk-read response decoder tests (Bug 17).
 *
 ...
 */
function buildBulkParamReadFrame(
...
describe('decodeBulkParamResponse', () => {
  it('returns null for non-response frame types', () => {
    ...
    expect(decodeBulkParamResponse(data)).toBeNull();
  });
  ...
  it('decodes baseWeight (uint16) from a single-param response', () => {
    const data = buildBulkParamReadFrame([
      { paramIdHex: ParamIdHex.BASE_WEIGHT, valueBytes: [75, 0] },
    ]);
  ...
  it('drops invalid trainingMode values (per VALID_TRAINING_MODES guard)', () => {
    const data = buildBulkParamReadFrame([
      { paramIdHex: ParamIdHex.TRAINING_MODE, valueBytes: [99] },
    ]);
    const result = decodeBulkParamResponse(data);
...
describe('identifyMessageType / decodeNotification — cmd=0x0F integration', () => {
  it('classifies a bulk-read response as `cmd_0f_bulk_response`', () => {
    const data = buildBulkParamReadFrame([
