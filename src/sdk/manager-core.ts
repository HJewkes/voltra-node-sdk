 ...
 * VoltraManagerCore
 *
 * Platform-agnostic core of the Voltra SDK manager. Owns every piece of
 * manager behavior — scan, connect, client lifecycle, events, name-prefix
 * resolution — and leaves the three platform-bound decisions (platform
 * detection, adapter-factory construction, host construction) to
 * subclasses.
 *
 * This module deliberately contains **no CommonJS `require` calls and no
 * Node-specific imports**, so a browser entry can extend it without
 * dragging Node peer dependencies into the bundle. The Node/universal
 * subclass lives in `voltra-manager.ts`; the browser subclass lives in
 * `entries/web.ts`.
 */
...
import { filterVoltraDevices, resolveDeviceNamePrefix } from '../voltra/models/device-filter';
...
export interface VoltraManagerOptions {
   ...
   *
   * Defaults to no name filtering — devices are identified by the BLE
   * service UUID, which every scan path filters on. Set a string to
   * restrict discovery to devices whose advertised name starts with it
   * (e.g. the factory default `VOLTRA_DEVICE_PREFIX`). `null` or `''`
   * explicitly disables filtering and skips the
   * `VOLTRA_DEVICE_NAME_PREFIX` environment variable.
   *
   * Renaming a Voltra from the vendor app changes its advertised name,
   * so a name filter will hide it. Prefer leaving this unset.
   */
...
/**
 * Platform-agnostic manager core. Subclasses supply platform detection,
 * the adapter factory, and the `BluetoothHost` construction.
 */
export abstract class VoltraManagerCore {
  ...
  /**
   * Effective device name prefix, resolved once at construction from
   * `options.deviceNamePrefix` then `VOLTRA_DEVICE_NAME_PREFIX`.
   * `undefined` means no name filtering (the default).
   */
  protected readonly deviceNamePrefix: string | undefined;
  ...
  constructor(options: VoltraManagerOptions = {}) {
    ...
    // Platform first — the name-prefix default depends on it.
    ...
    // Resolved before the factory runs — `createAdapterFactory` and the
    // host builder bake it into each adapter's BLE config.
    this.deviceNamePrefix = this.resolveManagerNamePrefix(options.deviceNamePrefix);

    this.adapterFactory = options.adapterFactory ?? this.createAdapterFactory(this.platform);

    // Phase 0: every flow now goes through a BluetoothHost. If the user
    // supplied one, use it directly; otherwise let the subclass build the
    // host that fits the resolved platform. Phase 1 introduces
    // `'node-noble'` which builds a `NobleHost` directly (no legacy
    // BLEAdapter shim — the noble peripheral implements `Peripheral`
    // natively). All other platforms wrap the adapter factory as a
    // `LegacyAdapterHost`.
    this.host = options.host ?? this.createHost(this.platform, this.adapterFactory);
  }
  ...
  /**
   * Detect the runtime platform when the caller did not specify one.
   */
  protected abstract detectPlatform(): Platform;

  /**
   * Build the legacy `BLEAdapter` factory for the resolved platform.
   * Implementations read `this.deviceNamePrefix`, which the constructor
   * resolves before calling this.
   */
  protected abstract createAdapterFactory(platform: Platform): AdapterFactory;

  /**
   * Build the `BluetoothHost` for the resolved platform. Only called when
   * the caller did not supply `options.host`.
   */
  protected abstract createHost(platform: Platform, factory: AdapterFactory): BluetoothHost;
  ...
  /**
   * The platform this manager resolved to, after auto-detection.
   * Useful when diagnosing discovery problems.
   */
  get resolvedPlatform(): Platform {
    return this.platform;
  }

  /**
   * The effective device name prefix, or `undefined` when scan results
   * are not name-filtered. Useful when diagnosing discovery problems.
   */
  get resolvedDeviceNamePrefix(): string | undefined {
  ...
  async scan(options: ScanOptions = {}): Promise<DiscoveredDevice[]> {
    ...
    // Per-scan override wins over the manager-level prefix; both default
    // to no name filtering.
    const namePrefix =
      options.deviceNamePrefix !== undefined
        ? resolveDeviceNamePrefix(options.deviceNamePrefix)
    ...
    try {
      // Hosts that honor `deviceNamePrefix` filter during the scan; the
      // legacy shim ignores it and is caught by the post-filter below.
      const discoveries = await this.host.scan({ timeout, deviceNamePrefix: namePrefix });
      ...
      // No-op unless a prefix was opted into — identity comes from the
      // service-UUID filter applied during the scan itself.
      this.discoveredDevices = filterVoltra ? filterVoltraDevices(devices, namePrefix) : devices;
  ...
  /**
   * Resolve the manager-level name prefix.
   *
   * Precedence: explicit option (including `null` to disable) →
   * `VOLTRA_DEVICE_NAME_PREFIX` → the platform default.
   */
  private resolveManagerNamePrefix(explicit: string | null | undefined): string | undefined {
    if (explicit !== undefined) return resolveDeviceNamePrefix(explicit);
    return resolveDeviceNamePrefix() ?? this.defaultNamePrefixFor(this.platform);
  }

  /**
   * Default name prefix per platform. Undefined (no name filtering)
   * everywhere except the legacy `'node'` backend.
   *
   * `NodeBLEAdapter` is a picker, not a scanner: `webbluetooth`'s
   * `requestDevice` selects the FIRST device passing the filter and stops
   * scanning. With no name filter it returns whatever advertises first —
   * observed on hardware 2026-07-28 returning a television. The `VTR-`
   * prefix is therefore load-bearing on that backend, so it stays on by
   * default there. Prefer `'node-noble'`, which enumerates properly and
   * identifies devices by advertised service UUID.
   */
  private defaultNamePrefixFor(platform: Platform): string | undefined {
    return platform === 'node' ? BLE.DEVICE_NAME_PREFIX : undefined;
  }
