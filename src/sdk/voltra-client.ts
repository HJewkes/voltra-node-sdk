...
export class VoltraClient {
   ...
   *   screen-switch + vendor-refresh pair at `+750 / +1750 / +3000 ms`. Advanced callers who
   ...
   * respond to the strength-arm primitive. Writing it while the
   * device is
   ...
   * commits via screen-switch action codes, which is the only correct
  ...
  // ===========================================================================
  //
  // Rowing commits via a screen-switch write followed by a vendor
  // state-refresh pulse, NOT via the strength-mode GO. The original SDK's `setMode(Rowing)` +
   ...
   *
   * Sends the existing workout-state = ROWING frame. After this
   ...
   *
   * Writes the screen-switch register with the action that picks the preset,
   ...
   * rowing-active, advances the current-screen and ongoing-UI registers,
   * and starts rowing telemetry.
   *
   * Note on distance presets: the device does not receive a native
   * target-distance register — the screen-switch write only selects the preset
   ...
   * `BP_SET_FITNESS_MODE` to strength-ready. Stops the poll loop
   ...
   *
   * The raw `assistMode` byte is preserved. This register has asymmetric-off
   * semantics: only the "on" code means on, and every other value should be
   * treated as off.
   *
