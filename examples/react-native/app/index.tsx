 ...
 *
 * Demonstrates using the Voltra SDK in a React Native app with Expo.
 */
...
import { NativeBLEAdapter, BLE, type DiscoveredDevice } from '@voltra/node-sdk';

export default function WorkoutScreen() {
  // Create adapter (memoized to prevent recreation)
  const adapter = useMemo(() => new NativeBLEAdapter({ ble: BLE }), []);
  ...
  const handleConnect = async (device: DiscoveredDevice) => {
    try {
      ...
      // Apply default weight after connection
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Error Display */}
      {clientError && (
        <Text style={styles.errorText}>{clientError.message}</Text>
      )}
    </SafeAreaView>
  );
}
